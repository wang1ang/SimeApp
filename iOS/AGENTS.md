...
- 候选更新必须重置两行到左侧；候选栏要终止惯性滚动并允许立即点按（`render()` 中已有处理）。
...
- 当前根 stack 顶部约束是 `constant: -10`，这是用户要求观察真机效果的试验值；若视觉异常，应先与用户确认再调整，不要默默恢复。
- 当前紧凑值：第一行 28pt；第一、二行间距 1pt；第一行字间距 3pt；第二行候选间距 6pt。
