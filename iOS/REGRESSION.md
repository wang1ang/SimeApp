...
11a. 微软双拼下每个已打满两键的音节韵母已锁定：多音节候选不得把已锁音节缩短为更短的韵母（如输 `xi`+`huan` 不得出现 `xi'hu`/`xi'hua` 路径）；仅末尾奇数键的未完成音节可继续扩展。单字候选不受此限制。过滤逻辑由 `iOS/Tests` 覆盖；此处只保留真模型真机回归（真实 Sime 模型下多音节双拼不得回吐被锁音节的短韵母候选）。
